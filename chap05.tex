...
% \begin{itemize}
% \item co ma contrast za ukol
% \item porovnavaci algoritmy, odkud jsme co ukradli a jak jsme to upravili, proc jsme vybrali zrovna tyto algoritmy
% \item flow graph, vstupy a vystupy komponent
% \item obecne zodpovednosti komponent
% \item rozdeleni do knihoven, build system, registry, modularita pro easy vymeneni algoritmu
...
This gives us the ability to once compute all the similarity scores and later generate multiple reports with different thresholds without the need to recompute anything.
...
Definition and theoretical background of TSED is described in \autoref{sec:ted}.
...
It behaves same as on trees.

The algorithm is implemented in the \texttt{algorithm\_ted\_pre} class.
The normalization of the computed scores and the enumeration of the tasks for the comparisons is implemented in the \texttt{algorithm\_distance} class, that inherits from the \texttt{algorithm} class.
...
If they are not, we sum the cost of transforming the current subtrees (which is already computed and stored in the database) and the cost of transforming the remaining forests (that is in $D$ after the end of the current subtrees).

Let us now closely look at the cell $D[2][2]$.
...
The cost of transforming the current subtrees (its the node $B$ from the left tree and the whole right tree) is already computed and stored in the database, and it is $3$.
...
We can see that the edit script contained only inserting the node $X$, what really makes a total distance of $1$ between the two trees.

\subsection{CrystalBLEU}
...
It completely ignores the most common $n$-grams, which are comming from the nature of the programming language and boilerplate code; differing from the original BLEU algorithm.

It is asymmetrical and computes the similarity score of a candidate with respect to a reference.
The need for symmetry is a common requirement and is solved for example in the BLEU algorithm by computing the score in both directions and taking the average \cite{sbleu10}.
We use a different approach and directly change the asymmetrical formulas to symmetrical ones. But there is still a lot of space for experiments.

We choose CrystalBLEU as one of the comparison algorithms because it was shown to be effective in the plagiarism experiments of Ebrahim and Joy \cite{ebrahim26} and is straightforward and reasonably fast.
...
From the frequencies counted for module roots, we can determine the most common ones and proclaim them as the crystals of the codebase.
The crystals are stored in a set of hash values.

We apply the crystalization process to the vectors of $n$-gram frequencies, removing the crystals from the vectors.
We store the crystalized vectors in memory and use them for the comparisons of the code fragments.
