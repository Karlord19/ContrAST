...
\subsection{Tree similarity edit distance}\label{sec:tsed}

Tree similarity edit distance (TSED) is a tree-based algorithm that computes the similarity based on the cost of transforming one tree into another using a set of edit operations.
...
The cost of the edit operations is crucial for the performance of TSED \cite{song24,changedistilling07}, and we need to pay special attention to the cost of the relabeling operation.
...
Apart from the database of the computed scores, each comparison is entirely independent of the others, and we can run multiple comparisons in parallel (we use OpenMP \cite{openmp} for parallelization).
...
We discuss this in more detail in \nameref{sec:name:future}.

The cost metric for the edit operations is based on the labels of the nodes.
We operate only on the core storage of \texttt{comparatree}, where we work directly with the labels of the AST nodes.
To take full advantage of AST node deduplication, we do not resolve concrete node instances, which means we cannot use instance-specific information such as identifier names or literal values.
We provide a simple interface for the cost metric that allows setting the cost of each edit operation based on the node labels.
We do not distinguish between deletion and insertion cost, ensuring that the metric remains symmetric.
...
The normalization of the computed scores and the enumeration of the tasks for the comparisons are implemented in the \texttt{algorithm\_distance} class, which inherits from the \texttt{algorithm} class.
...
The cost of transforming the current subtrees (which is the node $B$ from the left tree and the whole top tree) is already computed and stored in the database, and it is $3$.
...
We can trace back the operations that led to this distance by looking at the minimum cost operation in each cell.
Cell $D[0][0]$ has the minimum cost from the relabeling operation, so we relabel the root nodes of both trees (node $A$ from the left tree to the node $A$ from the top tree).
We then move to the diagonal cell $D[1][1]$, which has the minimum cost from the jump operation, so we transform the current subtrees (node $B$ from the left tree to the node $B$ from the top tree).
...
We move to the cell $D[2][3]$, which has the minimum cost from the relabeling operation, so we relabel the node $C$ from the left tree to the node $C$ from the top tree.
...
CrystalBLEU \cite{crystalbleu23} is a token-based algorithm adapted for trees by first converting them into token sequences via preorder traversal.
It measures the similarity of two code fragments based on the number of their shared $n$-grams.
An $n$-gram is a contiguous sequence of $n$ tokens, and the algorithm uses all $n$-grams up to $n=4$.
Unlike the original BLEU algorithm, CrystalBLEU completely ignores the most common $n$-grams, which typically stem from the nature of the programming language and boilerplate code.
...
The comparisons are independent of each other, and we can run multiple comparisons in parallel (we use OpenMP \cite{openmp} for parallelization).
...
The original BLEU algorithm finally multiplies the score by a brevity penalty, which penalizes candidates that are significantly shorter than the reference.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{img/ngrams-int.eps}
\caption{Occurrences of different $n$-grams. We choose the cutoff for the crystals and compare code fragments without them, using only the interesting $n$-grams on the right side of the line.}
\label{fig:ngrams-int}
\end{figure}

We provide a simple visualization tool for $n$-grams, as shown in \autoref{fig:ngrams-int}, displaying their occurrences across the dataset.
The vertical cutoff line separates the most common $n$-grams (the crystals) from the remaining $n$-grams (the interesting ones).
This visualization directly helps the user pick the right number of crystals for a given codebase, making it easy to find a good balance between filtering out boilerplate code and keeping enough $n$-grams for accurate comparisons.
