...
The reports highlight similar code fragments between different files, grouped by an author of the submission.
...
A string-based plagiarism-detection tool called Comparatrix, developed by RNDr. David Bednárek, Ph.D., is currently used in ReCodEx for C++ submissions.
...
A syntax-driven tool for detecting plagiarism in ReCodEx focused on Python submissions was developed by Jakub Saksa \cite{saksa20}.
It lets Python to parse the code and then hashes the resulting ASTs to find identical code fragments.
This approach is more robust than the string-based one, but it still only detects identical code.

At the end of each day, a batch of new files is sent to the plagiarism-detection tools, and the reports focusing on the new files (and their similarity to the new or old files) are generated and sent back to ReCodEx.
