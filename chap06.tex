...
Because the current implementation computes and stores similarity scores for all pairs of core nodes, the required computation time and storage space grow quadratically with the total number of core nodes in the \texttt{comparatree} instance.
Regarding these space requirements, the benchmarking process ran until the \texttt{comparatree} archive reached approximately 80~MB, at which point the temporary database exhausted the available storage on the test machine and the process terminated.
Nevertheless, the recorded data demonstrates that the algorithm scales efficiently, with each newly appended batch being processed in under 1 minute.
