...
For CrystalBLEU, we applied the \texttt{conservative} abstraction, whereas for TSED, we used the \texttt{extreme} abstraction alongside the \texttt{extremescs} cost metric (as detailed in \autoref{sec:abstraction} and \autoref{sec:cost-metric}).
...
The performance tests were executed on a virtual machine allocated 8 vCPUs on a host system powered by an Intel(R) Xeon(R) Gold 6326 CPU @ 2.90GHz.
The benchmarks evaluate execution times across assignment batches collected from three courses at our faculty: Programming in C++ (\texttt{int}), Advanced Programming in C++ (\texttt{adv}), and Computer Systems (\texttt{ard}), as defined in \autoref{sec:comparatree-statistics}.
...
For CrystalBLEU, we used the \texttt{conservative} abstraction configuration, and for TSED, we used the \texttt{extreme} abstraction configuration, matching the evaluation setup in \autoref{sec:evaluation}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{img/performance-ted-adv-ard-int.eps}
  \caption{Performance of TSED across different assignment batches.}
...
\begin{figure}[ht]
  ...
  \caption{Performance of CrystalBLEU across different assignment batches.}
...
\begin{figure}[ht]
  ...
  \caption{Performance of CrystalBLEU on a growing archive of cumulative batches.}
